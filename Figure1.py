...
if __name__ == "__main__":
    ...
    fig, ax = pl.subplots()
    sns.tsplot(test_256, test_time, ax=ax, color="C0", n_boot=1000, condition="256 HU")
    ...
    sns.tsplot(bf_evolved, centers, n_boot=1000, color="C2", err_style="ci_band", condition="p(Swim) control")
    ax.set_xlim(23, 36)
    ...
    centers = a.temp_convert(bins[:-1]+np.diff(bins), "r")
    ...
    for i, p in enumerate(paths_512):
        ...
        naive[i, :] = a.bin_simulation(pos_n, bins, "r")
        pos_t = ana.run_simulation(mpath(p), "r", "trained")
        trained[i, :] = a.bin_simulation(pos_t, bins, "r")
        pos_e = ana.run_simulation(mpath(p), "r", "bfevolve")
        evolved[i, :] = a.bin_simulation(pos_e, bins, "r")
    fig, ax = pl.subplots()
    ...
    sns.tsplot(evolved, centers, n_boot=1000, condition="p(Swim) control", color="C2")
